...
const WorkspaceStatus = () => {
      ...
      label: (
        <Space style={{ width: '100%', justifyContent: 'space-between' }}>
          <Space>
            ...
            {workspace.type === 'git' && wsSyncInfo?.syncing && <SyncOutlined spin style={{ fontSize: 12 }} />}
            {hasError && <ExclamationCircleOutlined style={{ color: '#ff4d4f', fontSize: 12 }} />}
          </Space>
  ...
  if (teamWorkspaces.length > 0) {
    ...
    finalMenuItems.push({ key: 'team-header', type: 'group' as const, label: 'Team Workspaces' }, ...teamWorkspaces);
  }
