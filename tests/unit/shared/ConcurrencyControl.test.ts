...
import { Mutex, Semaphore, ConcurrentMap, ConcurrentSet, RequestDeduplicator } from '../../../src/shared/ConcurrencyControl';
...
describe('Mutex', () => {
    it('allows sequential access', async () => {
        ...
        await mutex.withLock(async () => { results.push(1); });
        await mutex.withLock(async () => { results.push(2); });
    ...
    it('queues concurrent access', async () => {
        ...
        const p2 = mutex.withLock(async () => { results.push(2); });
