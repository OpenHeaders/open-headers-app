...
const VirtualizedFilterableTable = forwardRef<unknown, VirtualizedFilterableTableProps & Record<string, unknown>>(
  (allProps, ref) => {
    const {
      columns,
      dataSource,
    ...
    const Row = useCallback(
      ({ index, style }: { index: number; style: React.CSSProperties }) => {
        ...
        const key: React.Key =
          typeof rowKey === 'function' ? rowKey(record) : rowKey ? (record[rowKey] as React.Key) : index;
                  ...
                  onChange={(e) => {
                    if (rowSelection.type === 'radio') {
                      ...
                      const newSelectedRows = processedData.filter((item, itemIdx) => {
                        const itemKey: React.Key =
                          typeof rowKey === 'function' ? rowKey(item) : rowKey ? (item[rowKey] as React.Key) : itemIdx;
          ...
          <div className="virtual-table-header-cell selection-cell" style={{ width: 40 }}>
            {rowSelection.type !== 'radio' && (
              <input
                type="checkbox"
                checked={
                  (rowSelection?.selectedRowKeys?.length ?? 0) === processedData.length && processedData.length > 0
                }
                ref={(checkbox) => {
                  if (checkbox) {
                    checkbox.indeterminate =
                      (rowSelection?.selectedRowKeys?.length ?? 0) > 0 &&
                      (rowSelection?.selectedRowKeys?.length ?? 0) < processedData.length;
                  }
                }}
                onChange={(e) => {
                  const checked = e.target.checked;
