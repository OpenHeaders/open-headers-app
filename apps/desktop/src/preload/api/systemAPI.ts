...
const systemAPI = {
  ...
  openAppPath: (pathKey: string): Promise<{ success: boolean; error?: string }> =>
    ipcRenderer.invoke('openAppPath', pathKey),
  getSystemTimezone: (): Promise<{ timezone: string; offset: number; method: string }> =>
    ipcRenderer.invoke('getSystemTimezone'),

  checkScreenRecordingPermission: (): Promise<{ success: boolean; hasPermission?: boolean }> =>
