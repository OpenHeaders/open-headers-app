...
class GitErrorHandler {
   ...
   */
  classifyError(error: EnhancedError): ErrorType {
   ...
   */
  requiresUserAction(errorType: ErrorType): boolean {
    const userActionTypes: ErrorType[] = [
      ERROR_TYPES.AUTH_ERROR,
      ERROR_TYPES.CONFLICT_ERROR,
      ERROR_TYPES.PERMISSION_ERROR,
      ERROR_TYPES.GIT_NOT_FOUND,
   ...
   */
  format(handledError: HandledError): string {
    let formatted = `Error: ${handledError.message}\n`;
