...
export async function addSource(ctx: StateContext, sourceData: Source): Promise<Source> {
    const isDuplicate = ctx.state.sources.some(src =>
    ...
    const deps = sourceData.sourceType === 'http'
    ...
    if (newSource.sourceType === 'http' && ctx.sourceRefreshService) {
        ctx.sourceRefreshService.updateSource(newSource).catch(e =>
            log.warn(`Failed to register source ${newSource.sourceId} with refresh service:`, errorMessage(e))
        );
    }
...
export async function updateSource(ctx: StateContext, sourceId: string, updates: SourceUpdate): Promise<Source | null> {
    ...
    if (hasConfigChange && updatedSource && (updatedSource as Source).sourceType === 'http' && ctx.sourceRefreshService) {
        ctx.sourceRefreshService.updateSource(updatedSource as Source).catch(e =>
            log.warn(`Failed to sync source config to refresh service:`, errorMessage(e))
        );
    }
...
export async function removeSource(ctx: StateContext, sourceId: string): Promise<void> {
    ...
    if (ctx.sourceRefreshService) {
        ...
        ctx.sourceRefreshService.removeSourcesNotIn(httpIds).catch(e =>
            log.warn('Failed to clean up refresh service after source removal:', errorMessage(e))
        );
    }
}

export async function updateSourceFetchResult(ctx: StateContext, sourceId: string, result: {
    content: string;
...
export async function addHeaderRule(ctx: StateContext, ruleData: Partial<HeaderRule>): Promise<void> {
    const newRule: HeaderRule = { ...ruleData, id: Date.now().toString(), createdAt: new Date().toISOString() } as HeaderRule;
