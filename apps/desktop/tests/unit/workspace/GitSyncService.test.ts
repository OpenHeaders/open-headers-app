...
function isPermissionError(errorMessage: string): boolean {
  return (
    errorMessage.includes('permission') || errorMessage.includes('forbidden') || errorMessage.includes('unauthorized')
  );
}
...
describe('GitSyncService — pure logic', () => {
  ...
  describe('getWorkspaceRepoDir()', () => {
    it('builds tempDir/workspace-{id} path', () => {
      expect(getWorkspaceRepoDir('/tmp/git', 'abc-123')).toBe('/tmp/git/workspace-abc-123');
    });
