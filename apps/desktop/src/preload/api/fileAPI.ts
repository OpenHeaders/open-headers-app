...
const fileAPI = {
  ...
  readFile: (filePath: string, encoding: string): Promise<string | Buffer> =>
    ipcRenderer.invoke('readFile', filePath, encoding),
  ...
  watchFile: (sourceId: string, filePath: string): Promise<string> =>
    ipcRenderer.invoke('watchFile', sourceId, filePath),
  ...
  saveToStorage: (filename: string, content: string): Promise<void> =>
    ipcRenderer.invoke('saveToStorage', filename, content),
  ...
  deleteDirectory: (dirPath: string): Promise<{ success: boolean; error?: string }> =>
    ipcRenderer.invoke('deleteDirectory', dirPath),
