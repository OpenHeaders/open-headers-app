...
const FormHeader: React.FC<FormHeaderProps> = ({ mode, headerType, setHeaderType, envVarValidation, envContext }) => {
  ...
  const validateHeaderName = useCallback(
    (_: unknown, value: string) => {
      ...
      if (!hasEnvVars) {
        ...
        if (
          FORBIDDEN_HEADERS.some((forbidden) => {
    ...
    <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', marginBottom: 16 }}>
      {mode === 'generic' ? (
        <Form.Item
          ...
          extra={
            envVarValidation.headerName?.hasVars && (
              <Text type="secondary" style={{ fontSize: 12 }}>
                <InfoCircleOutlined /> Uses environment variables:{' '}
                {envVarValidation.headerName.usedVars.map((v: string) => `{{${v}}}`).join(', ')}
              </Text>
            )
          }
        >
        ...
        <Form.Item
          ...
          extra={
            envVarValidation.cookieName?.hasVars && (
              <Text type="secondary" style={{ fontSize: 12 }}>
                <InfoCircleOutlined /> Uses environment variables:{' '}
                {envVarValidation.cookieName.usedVars.map((v: string) => `{{${v}}}`).join(', ')}
              </Text>
            )
          }
        >
          <Input placeholder="Cookie Name" size="small" />
        </Form.Item>
      )}
      ...
      <Form.Item name="tag" style={{ marginBottom: 0, width: 180 }}>
        <Input placeholder="Tag (optional)" size="small" maxLength={20} />
      </Form.Item>
    </div>
  );
};
