    ...
    },
    {
      ...
      render: (ua: string | undefined) => {
        ...
        if (ua.includes('PowerShell/')) {
          const m = ua.match(/(?:Windows)?PowerShell\/(\d+\.\d+)/);
          label = m ? `PowerShell/${m[1]}` : 'PowerShell';
        } else if (ua.includes('Edg/')) {
          const m = ua.match(/Edg\/([\d.]+)/);
          label = m ? `Edge/${trimVer(m[1])}` : 'Edge';
        } else if (ua.includes('OPR/')) {
          const m = ua.match(/OPR\/([\d.]+)/);
          label = m ? `Opera/${trimVer(m[1])}` : 'Opera';
        } else if (ua.includes('Chrome/') && !ua.includes('Edg/')) {
          const m = ua.match(/Chrome\/([\d.]+)/);
          label = m ? `Chrome/${trimVer(m[1])}` : 'Chrome';
        } else if (ua.includes('Firefox/')) {
          const m = ua.match(/Firefox\/([\d.]+)/);
          label = m ? `Firefox/${trimVer(m[1])}` : 'Firefox';
        } else if (ua.includes('Safari/') && !ua.includes('Chrome/')) {
          const m = ua.match(/Version\/([\d.]+)/);
          label = m ? `Safari/${trimVer(m[1])}` : 'Safari';
        }
      ...
      if (record.bodySummary) {
        details.push(
          <div key="body">
            ...
            >
              {JSON.stringify(record.bodySummary, null, 2)}
            </pre>
          </div>,
        );
      }
    ...
    <Card style={{ marginTop: 16 }}>
      <Space direction="vertical" style={{ width: '100%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Space>
            <Button size="small" icon={<DownloadOutlined />} onClick={onExportLogs} disabled={logs.length === 0}>
              Export
            </Button>
            <Button size="small" icon={<ReloadOutlined />} onClick={onRefresh}>
              Refresh
            </Button>
            <Button size="small" icon={<DeleteOutlined />} onClick={onClearLogs} disabled={allLogs.length === 0}>
              Clear
            </Button>
          </Space>
        </div>
