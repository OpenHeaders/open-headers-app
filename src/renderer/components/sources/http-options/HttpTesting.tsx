...
import type { EnvironmentContextLike, HttpProgressCallback, TestResponseContent, HttpRequestSpec } from '../../../../types/http';
...
function buildRequestSpec(form: FormInstance, values: HttpFormValues, totpState: TotpState, effectiveSourceId: string, workspaceId: string, sourcePath?: string | null, sourceMethod?: string | null): HttpRequestSpec {
    ...
    if (Array.isArray(formHeaders) && formHeaders.length > 0) {
        ...
        headers = values.requestOptions.headers.filter(h => h && h.key).map(h => ({ key: h.key, value: h.value || '' }));
    }
    ...
    if (Array.isArray(formQueryParams) && formQueryParams.length > 0) {
        ...
        queryParams = values.requestOptions.queryParams.filter(p => p && p.key).map(p => ({ key: p.key, value: p.value || '' }));
    }
