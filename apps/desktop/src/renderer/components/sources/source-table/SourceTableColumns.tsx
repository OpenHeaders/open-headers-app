    ...
    render: (_value: unknown, record: Source) => {
      ...
      return (
        <Space size={4} direction="vertical" align="start">
          ...
          {type === 'http' &&
            record.activationState === 'waiting_for_deps' &&
            (record.missingDependencies?.length ?? 0) > 0 && (
    ...
    render: (_value: unknown, record: Source) => {
      ...
      return (
        <div className="source-content-cell" style={{ maxHeight: '60px', fontSize: '11px' }}>
          <Text ellipsis={{ tooltip: content }}>{trimContent(content)}</Text>
        </div>
      );
    },
  },
          ...
          (() => {
            ...
            const cbTooltip =
              isCircuitOpen && circuitBreaker ? (
                <div>
                  <div>
                    Auto-refresh temporarily disabled after{' '}
            ...
            return (
              <>
                <div className={`refresh-status ${record.refreshOptions?.enabled ? 'active' : ''}`}>
                  {cbTooltip ? (
                    <Tooltip title={cbTooltip} placement="top">
                      {statusContent}
                    </Tooltip>
                  ) : (
                    statusContent
