...
describe('ProxyService', () => {
    ...
    describe('resolveEnvironmentVariables()', () => {
        it('replaces {{VAR}} with environment variable value', () => {
            ...
            expect(proxyService.resolveEnvironmentVariables('Bearer {{API_KEY}}')).toBe('Bearer ohk_live_4eC39HqLyjWDarjtT1zdp7dc');
        });
    ...
    describe('source management', () => {
        ...
        it('updateSources skips entries with null sourceContent', () => {
            proxyService.updateSources([
                makeSource({ sourceId: 'null-content', sourceContent: null }),
            ]);
    ...
    describe('handleRequest() URL routing', () => {
        ...
        it('strips leading slash from URL (e.g., /https://...)', async () => {
            ...
            let capturedUrl = '';
            proxyService.doProxy = vi.fn((_req, _res, url) => { capturedUrl = url; }) as typeof proxyService.doProxy;
        ...
        it('responds to OPTIONS with CORS headers', async () => {
            const res = createMockResponse();
            await proxyService.handleRequest(
                { method: 'OPTIONS', url: '/', headers: { 'access-control-request-headers': 'Authorization, X-Custom-Header' } } as unknown as Parameters<typeof proxyService.handleRequest>[0],
                res as unknown as Parameters<typeof proxyService.handleRequest>[1],
            );
    ...
    describe('rule CRUD via ProxyService', () => {
        it('saveRule returns success', async () => {
            const result = await proxyService.saveRule(makeProxyRule({
                id: 'new-rule',
                headerName: 'X-Test',
