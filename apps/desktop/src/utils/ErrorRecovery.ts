...
class ErrorRecovery {
  ...
  retryAttempts: Map<string | undefined, number>;
  maxRetries: number;
  baseDelay: number;
  maxDelay: number;
   ...
   */
  getStrategy(errorType: ErrorType): StrategyFn {
   ...
   */
  async handleNetworkError(error: Error, context: RecoveryContext, retryFn: RetryFn): Promise<unknown> {
   ...
   */
  async handleAuthError(error: Error, context: RecoveryContext, retryFn: RetryFn): Promise<unknown> {
   ...
   */
  async handleTimeoutError(error: Error, context: RecoveryContext, retryFn: RetryFn): Promise<unknown> {
    if (retryFn && this.shouldRetry(context.operationId)) {
      // Increase timeout for retry
      if (context.updateTimeout && context.timeout) {
   ...
   */
  async handleServerError(error: Error, context: RecoveryContext, retryFn: RetryFn): Promise<unknown> {
    if (retryFn && this.shouldRetry(context.operationId)) {
      const delay = this.getRetryDelay(context.operationId);
   ...
   */
  async handleConflictError(error: Error, context: RecoveryContext, retryFn: RetryFn): Promise<unknown> {
   ...
   */
  async handleUnknownError(error: Error, context: RecoveryContext, retryFn: RetryFn): Promise<unknown> {
   ...
   */
  async waitForNetwork(maxWait: number = 30000): Promise<boolean> {
