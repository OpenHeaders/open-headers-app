      ...
      title={
        <Space>
          <Tag color={getTypeColor(selectedEntry?.type ?? '')}>
            {selectedEntry?.type === 'localStorage'
              ? 'LOCAL STORAGE'
              : selectedEntry?.type === 'sessionStorage'
                ? 'SESSION STORAGE'
                : 'COOKIE'}
          </Tag>
          <Tag color={getActionColor(selectedEntry?.action ?? '')}>{selectedEntry?.action?.toUpperCase()}</Tag>
    ...
    >
      <div>
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <Space>
            <Button icon={<CopyOutlined />} disabled={!canCopyOldValue()} onClick={handleCopyOldValue}>
              {selectedEntry.action === 'clear' ? 'Copy Cleared Entries' : 'Copy Old Value'}
            </Button>

            <Button icon={<CopyOutlined />} disabled={!canCopyNewValue()} onClick={handleCopyNewValue}>
              Copy New Value
            </Button>
          </Space>
        </Space>
