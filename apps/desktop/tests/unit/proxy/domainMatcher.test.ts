...
describe('DomainMatcher', () => {
  describe('matches()', () => {
    ...
    describe('full URL patterns (*://...)', () => {
      it('matches full URL pattern with wildcards', () => {
        expect(DomainMatcher.matches('https://api.openheaders.io/v2/oauth/token', '*://api.openheaders.io/*')).toBe(
          true,
        );
      });

      it('matches http protocol variant', () => {
        expect(DomainMatcher.matches('http://api.openheaders.io/legacy/endpoint', '*://api.openheaders.io/*')).toBe(
          true,
        );
      });
      ...
      it('matches URL pattern with specific path prefix', () => {
        expect(
          DomainMatcher.matches('https://api.openheaders.io/v2/tokens/refresh', '*://api.openheaders.io/v2/*'),
        ).toBe(true);
      });

      it('does not match URL pattern with non-matching path', () => {
        expect(DomainMatcher.matches('https://api.openheaders.io/v1/tokens', '*://api.openheaders.io/v2/*')).toBe(
          false,
        );
      });
    });
    ...
    describe('edge cases', () => {
      ...
      it('handles URL with fragment', () => {
        expect(DomainMatcher.matches('https://docs.openheaders.io/page#section-3', 'docs.openheaders.io')).toBe(true);
      });

      it('handles URL with authentication credentials', () => {
        expect(DomainMatcher.matches('https://user:pass@api.openheaders.io/endpoint', 'api.openheaders.io')).toBe(true);
      });

      it('handles domain with hyphen in subdomain', () => {
        expect(DomainMatcher.matches('https://us-east-1.api.openheaders.io/v1', '*.openheaders.io')).toBe(true);
      });

      it('handles very long URL', () => {
        const longPath = '/segment'.repeat(100);
        expect(DomainMatcher.matches(`https://api.openheaders.io${longPath}`, 'api.openheaders.io')).toBe(true);
      });
    });
  });
  ...
  describe('matchesAny()', () => {
    ...
    it('returns true when URL matches one of multiple domains', () => {
      const domains = ['api.openheaders.io', 'auth.internal.openheaders.io', '*.partners.openheaders.io'];
    ...
    it('returns false when URL matches none of the domains', () => {
      const domains = ['api.openheaders.io', 'auth.internal.openheaders.io:8443', '*.partners.openheaders.io'];
    ...
    it('matches with mixed domain pattern types', () => {
      const domains = ['*.openheaders.io', 'localhost:3000', '192.168.1.1:8080', '*://partners.openheaders.io/*'];
