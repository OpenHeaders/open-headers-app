...
class WorkspaceCreationController {
  ...
  async createWorkspace(formData: WorkspaceFormValues) {
    ...
    try {
      ...
      const authData =
        formData.type === 'team' ? await prepareAuthData(formData, formData.authType || 'none') : undefined;
  ...
  handleStateChange(stateData: StateChangeData) {
    ...
    switch (state) {
      ...
      case WORKSPACE_CREATION_STATES.ROLLBACK:
        ...
        this.stateMachine
          .executeRollback((action) => this.executeRollbackAction(action))
          .catch((error) => {
  ...
  reset() {
    ...
    const shouldAbort =
      currentState !== WORKSPACE_CREATION_STATES.COMPLETED &&
