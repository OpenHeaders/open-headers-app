...
class AppStateMachineImpl extends EventEmitter {
  ...
  canTransition(event: string): boolean {
    ...
    return transitions && Object.hasOwn(transitions, event);
  }
  ...
  serversReady(servers: StateTransitionData['servers'] = {}): boolean {
    return this.transition('SERVERS_READY', { servers });
  }
  workspaceChange(workspace: StateTransitionData['workspace'] = {}): boolean {
  ...
  isShuttingDown(): boolean {
    return this.currentState === AppStates.SHUTTING_DOWN || this.currentState === AppStates.TERMINATED;
  }
...
export { AppStateMachine, AppStateMachineImpl };
