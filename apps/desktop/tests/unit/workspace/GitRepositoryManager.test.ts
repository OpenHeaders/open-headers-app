...
describe('GitRepositoryManager', () => {
  ...
  describe('pushRepository()', () => {
    ...
    it('always cleans up auth resources even on push failure', async () => {
      ...
      await expect(manager.pushRepository({ repoDir: REPO_DIR, branch: 'main' })).rejects.toThrow('push rejected');
  ...
  describe('ensureCleanDirectory()', () => {
    ...
    it('throws when directory is not empty', async () => {
      ...
      >);
      vi.spyOn(fs.promises, 'readdir').mockResolvedValue(['file.txt'] as unknown as Awaited<
        ReturnType<typeof fs.promises.readdir>
      >);

      await expect(manager.ensureCleanDirectory('/existing/dir')).rejects.toThrow('is not empty');
