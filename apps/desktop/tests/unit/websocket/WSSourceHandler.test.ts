...
describe('WSSourceHandler', () => {
  ...
  describe('_hasSourceContentChanged', () => {
    it('returns true when current sources is empty and new has items', () => {
      mockService.sources = [];
      expect(
        handler._hasSourceContentChanged([
    ...
    it('returns true when lengths differ', () => {
      mockService.sources = [makeSource({ sourceId: 'src-1', sourceContent: 'token-1' })];
      expect(
        handler._hasSourceContentChanged([
    ...
    it('returns true when a source has different sourceId', () => {
      mockService.sources = [makeSource({ sourceId: 'src-original', sourceContent: 'content' })];
      expect(
        handler._hasSourceContentChanged([makeSource({ sourceId: 'src-replaced', sourceContent: 'content' })]),
      ).toBe(true);
    });

    it('treats missing sourceContent as empty string', () => {
      mockService.sources = [makeSource({ sourceId: 'src-1', sourceContent: undefined })];
      expect(handler._hasSourceContentChanged([makeSource({ sourceId: 'src-1', sourceContent: undefined })])).toBe(
        false,
      );
    });

    it('detects change from empty to non-empty content', () => {
      mockService.sources = [makeSource({ sourceId: 'src-1', sourceContent: '' })];
      expect(
        handler._hasSourceContentChanged([
    ...
    it('treats null sourceContent same as empty string', () => {
      mockService.sources = [makeSource({ sourceId: 'src-1', sourceContent: null })];
      expect(handler._hasSourceContentChanged([makeSource({ sourceId: 'src-1', sourceContent: '' })])).toBe(false);
    });

    it('detects change from null to non-empty content', () => {
      mockService.sources = [makeSource({ sourceId: 'src-1', sourceContent: null })];
      expect(handler._hasSourceContentChanged([makeSource({ sourceId: 'src-1', sourceContent: 'new-content' })])).toBe(
        true,
      );
    });
  ...
  describe('updateSources', () => {
    it('broadcasts when source content changes', () => {
      mockService.sources = [makeSource({ sourceId: 'src-1', sourceContent: 'old' })];
      handler.updateSources([makeSource({ sourceId: 'src-1', sourceContent: 'new' })]);
    ...
    it('updates service sources reference', () => {
      const newSources = [makeSource({ sourceId: 'src-new', sourceContent: 'new-content' })];
    ...
    it('also broadcasts rules when content changes', () => {
      mockService.sources = [];
      handler.updateSources([makeSource({ sourceId: 'src-1', sourceContent: 'content' })]);
