 ...
 * @returns Array of workflow recording objects
 */
 ...
 * @param recordingId - ID of the workflow recording to delete
 ...
 * @param applyHighlight - Highlight function from navigation context
 * @param targetType - Target type for highlighting
 * @param itemId - ID of the item to highlight
 * @param delay - Delay before applying highlight (default: 500ms)
 */
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (!window.electronAPI?.saveUploadedRecording) {
          const saveError = new Error('Save functionality not available');
