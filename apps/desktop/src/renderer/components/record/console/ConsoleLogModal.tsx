...
import { format24HTimeWithMs, formatRelativeTimeWithSmallMs } from '@/renderer/utils';
...
const ConsoleLogModal = ({ visible, selectedLog, record, onClose, messageApi }: ConsoleLogModalProps) => {
  ...
  const formatTimestamp = () => {
    ...
    return (
      <span>
        {timeParts.main}
        <span style={{ fontSize: '0.85em', opacity: 0.7 }}>{timeParts.ms}</span> ({formattedAbsoluteTime.date}{' '}
        {formattedAbsoluteTime.time}
  ...
  return (
    <Modal
      title={
        <Space>
          <Tag color={getLogLevelColor(selectedLog.level)}>{selectedLog.level?.toUpperCase()}</Tag>
    ...
    >
      <div>
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <Button icon={<CopyOutlined />} onClick={handleCopyMessage}>
            Copy Message
          </Button>
        </Space>
      </div>
