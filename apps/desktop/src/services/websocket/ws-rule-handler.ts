...
class WSRuleHandler {
  ...
  broadcastRules(): void {
    ...
    log.info(
      `broadcastRules: ${inputCount} rules in, ${outputCount} after filtering, sent to ${clientCount} client(s)`,
    );
  }
   ...
   */
  _populateDynamicHeaderValues(rules: RulesCollection): {
    header: ProcessedHeaderRule[];
    request: RulesCollection['request'];
    response: RulesCollection['response'];
  } {
      ...
      .map((rule): ProcessedHeaderRule | null => {
        ...
        if (processed.isDynamic && processed.sourceId) {
          ...
          if (
            ...
            log.debug(
              `Rule "${rule.headerName}" pending — source ${processed.sourceId} ${source.activationState === 'waiting_for_deps' ? 'has unresolved dependencies' : 'content not yet fetched'}`,
            );
            return null;
          }
        }
  ...
  async handleToggleAllRules(ruleIds: string[], enabled: boolean): Promise<void> {
    ...
    if (this.onRuleToggleBatch) {
      try {
        await this.onRuleToggleBatch(ruleIds.map((id) => ({ ruleId: String(id), changes: { isEnabled: enabled } })));
