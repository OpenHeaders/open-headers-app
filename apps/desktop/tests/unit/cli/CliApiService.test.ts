...
describe('CliApiService', () => {
  ...
  describe('_validateAuth()', () => {
    ...
    it('rejects wrong token', () => {
      const req = {
        headers: { authorization: 'Bearer a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2' },
  ...
  describe('_redactString()', () => {
    ...
    it('redacts JWT tokens (eyJ prefix)', () => {
      expect(
        svc._redactString('eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ1c2VyQG9wZW5oZWFkZXJzLmlvIn0.sig'),
      ).toBe('***');
    });
