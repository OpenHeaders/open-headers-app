...
const RSA_PUBLIC_KEY = 'ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQDRndVLkklx2zfF+f/KBZ6ng8C0agYd1MadPooEVGcgMZ9oRqo1zhYCFegxrwqTSEwiItfAAZ8jscEYoDsZKNjwXclB1F8SwNDojWyntU4FDYtrgDlqE10bPuv0gMYrdZTAgMBAAE= deploy@openheaders.io';
...
describe('SSHAuthStrategy', () => {
    ...
    describe('extractHostname()', () => {
        ...
        it('extracts hostname from git@github.com URL', () => {
            expect(strategy.extractHostname('git@github.com:OpenHeaders/open-headers-app.git'))
                .toBe('github.com');
        });

        it('extracts hostname from git@gitlab.openheaders.io URL', () => {
            expect(strategy.extractHostname('git@gitlab.openheaders.io:platform/shared-headers.git'))
                .toBe('gitlab.openheaders.io');
        });

        it('extracts hostname from HTTPS URL', () => {
            expect(strategy.extractHostname('https://github.com/OpenHeaders/open-headers-app.git'))
                .toBe('github.com');
        });

        it('extracts hostname from HTTPS URL with port', () => {
            expect(strategy.extractHostname('https://gitlab.openheaders.io:8443/repo.git'))
                .toBe('gitlab.openheaders.io');
        });

        it('throws for completely invalid URL', () => {
            expect(() => strategy.extractHostname('???'))
                .toThrow('Failed to extract hostname');
        });
    });
    ...
    describe('convertToSshUrl()', () => {
        ...
        it('converts HTTPS GitHub URL to SSH with custom host', () => {
            const result = strategy.convertToSshUrl(
        ...
        it('updates existing SSH URL with custom host', () => {
            const result = strategy.convertToSshUrl(
                'git@github.com:OpenHeaders/open-headers-app.git',
                keyHash
            );
            expect(result).toBe(`git@${keyHash}.git:OpenHeaders/open-headers-app.git`);
        });
        ...
        it('throws for URL with insufficient path parts (no owner/repo)', () => {
            expect(() => strategy.convertToSshUrl('https://github.com/only-one', keyHash))
    ...
    describe('setup()', () => {
        it('writes key files and returns SSH URL with cleanup function', async () => {
            ...
            const result = await strategy.setup(
                'https://github.com/OpenHeaders/open-headers-app.git',
                { privateKey: RSA_PRIVATE_KEY, publicKey: RSA_PUBLIC_KEY }
            );
        ...
        it('throws when private key is missing', async () => {
            ...
            ).rejects.toThrow('SSH private key is required');
        });
        ...
        it('sets SSH_ASKPASS_REQUIRE when passphrase is provided', async () => {
            ...
            const result = await strategy.setup(
                'https://github.com/OpenHeaders/open-headers-app.git',
                { privateKey: RSA_PRIVATE_KEY, passphrase: 'my-secure-passphrase' }
            );
