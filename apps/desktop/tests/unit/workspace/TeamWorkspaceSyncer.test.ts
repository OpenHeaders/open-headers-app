...
describe('TeamWorkspaceSyncer', () => {
  ...
  describe('hasLocalChanges', () => {
    ...
    it('calls executor with correct command', async () => {
      ...
      expect(deps.executor.execute).toHaveBeenCalledWith('status --porcelain', { cwd: '/my/repo' });
    });
  });
  ...
  describe('getCommitCount', () => {
    ...
    it('calls executor with correct command', async () => {
      ...
      expect(deps.executor.execute).toHaveBeenCalledWith('rev-list --count origin/main..HEAD', { cwd: '/repo' });
    });
  });
  ...
  describe('checkSyncStatus', () => {
    ...
    it('throws on fetch failure (infrastructure errors propagate)', async () => {
      ...
      await expect(syncer.checkSyncStatus('/repo', 'main')).rejects.toThrow('network timeout');
    });
    ...
    it('retries fetch when branch not found on remote, then throws', async () => {
      ...
      await expect(syncer.checkSyncStatus('/repo', 'main')).rejects.toThrow("couldn't find remote ref main");
  ...
  describe('handlePull', () => {
    it('pulls and validates configuration', async () => {
      ...
      expect(progressCallback).toHaveBeenCalledWith(expect.objectContaining({ phase: 'pull' }));
      expect(progressCallback).toHaveBeenCalledWith(expect.objectContaining({ phase: 'validate' }));
    });
  });
  ...
  describe('handlePush', () => {
    it('pushes commits and returns success', async () => {
      ...
      expect(progressCallback).toHaveBeenCalledWith(expect.objectContaining({ phase: 'push' }));
    });
  });
  ...
  describe('handleConflict', () => {
    it('returns requiresManualResolution when autoResolve is false', async () => {
      const result = await syncer.handleConflict({
        ...
        status: {
          ...
          localCommit: 'local-abc',
          remoteCommit: 'remote-def',
          ahead: 1,
          behind: 2,
        },
        autoResolve: false,
    ...
    it('returns conflict status and aborts rebase when auto-resolve fails', async () => {
      ...
      expect(deps.executor.execute).toHaveBeenCalledWith('rebase --abort', { cwd: '/repo' });
    });
  ...
  describe('syncWorkspace', () => {
    ...
    it('calls progressCallback during sync', async () => {
      ...
      expect(progressCallback).toHaveBeenCalledWith(expect.objectContaining({ phase: 'status' }));
    });
  });
