...
export function useSourceRefresh({
  ...
  manualRefresh,
  addSource,
}: UseSourceRefreshDeps): UseSourceRefreshReturn {
  /**
   * Handle HTTP source refresh — delegates to main process via IPC
   */
  const handleHttpSourceRefresh = useCallback(
    async (sourceId: string, _updatedSource: Source | null = null): Promise<boolean> => {
      try {
  ...
  const refreshSourceWithHttp = useCallback(
  ...
  const handleAddSource = useCallback(
    async (sourceData: NewSourceData): Promise<boolean> => {
