...
import type { WorkspaceState, WebSocketServiceLike, ProxyServiceLike, SourceRefreshServiceLike } from './types';
...
export function syncToRefreshService(sources: Source[], refreshService: SourceRefreshServiceLike | null): void {
    ...
    for (const source of sources) {
        if (source.sourceType === 'http') {
            ...
            refreshService.updateSource(source).catch(e =>
                log.warn(`Failed to sync source ${source.sourceId} to refresh service:`, errorMessage(e))
            );
        }
    }
    refreshService.removeSourcesNotIn(httpIds).catch(e =>
