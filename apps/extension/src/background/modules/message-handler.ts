...
export function handleGeneralMessage(
  ...
  try {
    if (message.type === 'popupOpen') {
      ...
      try {
        ...
        setChunkedData('savedData', savedData, () => {
          if (browserAPI.runtime.lastError) {
            logger.info(
              'MessageHandler',
              'Error saving savedData:',
              (browserAPI.runtime.lastError as chrome.runtime.LastError).message,
            );
      ...
      logger.info(
        'MessageHandler',
        'Background received sources update notification:',
        message.sources ? (message.sources as Source[]).length : 0,
        'sources at',
