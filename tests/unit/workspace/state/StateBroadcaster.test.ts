...
import type { WorkspaceState, WebSocketServiceLike, ProxyServiceLike, SourceRefreshServiceLike } from '../../../../src/services/workspace/state/types';
...
describe('broadcastToServices', () => {
    it('updates webSocketService sources/rules and broadcasts', () => {
        ...
        const ws: WebSocketServiceLike = {
            ...
            environmentHandler: { loadEnvironmentVariables: () => ({}), resolveTemplate: vi.fn(), setVariables: vi.fn(), clearVariableCache: vi.fn() },
        };
        const proxy: ProxyServiceLike = {
            switchWorkspace: vi.fn(), updateSources: vi.fn(), updateHeaderRules: vi.fn(),
            updateProxyRules: vi.fn(), updateEnvironmentVariables: vi.fn(), clearRules: vi.fn(),
        };
...
describe('sendProgressToRenderers', () => {
    it('sends progress data to all windows', () => {
        ...
        expect(mockSend).toHaveBeenCalledWith('workspace:switch-progress', {
            step: 'loading', progress: 50, label: 'Loading data...',
            isGitOperation: false, targetWorkspace: { id: 'ws-1', name: 'Test', type: 'personal' },
        });
    });
});
