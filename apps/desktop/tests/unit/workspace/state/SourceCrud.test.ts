...
describe('addSource', () => {
    ...
    it('throws on duplicate source', async () => {
        ...
        await expect(addSource(ctx, httpSource({ sourcePath: 'https://api.openheaders.io/data' }))).rejects.toThrow('Source already exists');
    });
    ...
    it('evaluates dependencies for HTTP sources', async () => {
        const ctx = createCtx({
            envResolver: { loadEnvironmentVariables: () => ({}), resolveTemplate: vi.fn(), setVariables: vi.fn(), clearVariableCache: vi.fn() },
        });
...
describe('removeSource', () => {
    it('removes the source from state', async () => {
        const ctx = createCtx();
        ctx.state.sources = [httpSource({ sourceId: '1' }), httpSource({ sourceId: '2', sourcePath: 'https://api.openheaders.io/other' })];
