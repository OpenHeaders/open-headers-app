...
vi.mock('../../src/utils/browser-api', () => ({
  storage: {
    local: {
      get: (...args: unknown[]) =>
        mockStorageLocalGet(...(args as [string[], (result: Record<string, unknown>) => void])),
...
describe('sources-store', () => {
  ...
  describe('setSourcesFromApp', () => {
    ...
    it('persists to storage.local', () => {
      ...
      expect(mockStorageLocalSet).toHaveBeenCalledWith({ dynamicSources: sources }, expect.any(Function));
    });
