  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      progressUnsubscribeRef.current =
        services!.gitService.onProgress?.((event) => {
          if (event.type === 'git-connection') {
            const summary = event.data.summary ?? [];
            setConnectionProgress(
              summary.map((item) => (typeof item === 'string' ? { step: item, status: 'done' } : item)),
            );
          }
        }) ?? null;
      ...
      return (
        <div style={{ width: '100%' }}>
          ...
          <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
            {canAbort && <Button onClick={onCancel}>Cancel</Button>}
          </Space>
        </div>
      );
    }
                      ...
                      <div style={{ marginBottom: 16 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr', gap: '12px', marginBottom: 12 }}>
                          <Tooltip
                            title={
                              inviteData
                                ? 'This field is pre-filled from the workspace invite and cannot be changed'
                                : ''
                            }
                          >
                            <Form.Item
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                          ...
                          >
                            <Form.Item name="gitBranch" label="Branch" style={{ marginBottom: 0 }}>
                          ...
                          >
                            <Form.Item name="gitPath" label="Config directory path" style={{ marginBottom: 0 }}>
                              <Input
                                disabled={!!inviteData}
                                placeholder="e.g., config/ or path/to/config/"
                                size="small"
                              />
                            </Form.Item>
                          </Tooltip>
            ...
            ? [
                {
                  ...
                  status: 'running',
                  progress:
                    progress.step && progress.total ? Math.round((progress.step / progress.total) * 100) : undefined,
                },
              ]
            : []
        }
