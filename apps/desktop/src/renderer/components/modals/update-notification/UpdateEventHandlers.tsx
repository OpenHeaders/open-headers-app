...
interface UpdateEventHandlersParams {
  notificationManager: {
    ...
    showAlreadyCheckingNotification: (isDownloading: boolean) => void;
    showUpdateAvailableNotification: (info: { version: string }, progress: number) => void;
    showDownloadProgressNotification: (percent: number) => void;
    showUpdateReadyNotification: (info: { version: string }) => void;
    showNoUpdatesNotification: () => void;
    showUpdateErrorNotification: (msg: string) => void;
...
export const createUpdateEventHandlers = ({
  ...
  const handleUpdateAlreadyDownloaded = (payload: { isManual?: boolean; info?: UpdateInfoEvent | null } = {}) => {
    ...
    debugLog(
      `${debugLabel} Received "update-already-downloaded" event (manual check: ${isManual}, version: ${info?.version})`,
    );
  ...
  const setupEventListeners = () => {
    ...
    const unsubscribeAlreadyInProgress =
      window.electronAPI.onUpdateCheckAlreadyInProgress?.(handleUpdateCheckAlreadyInProgress) || (() => {});
    const unsubscribeAlreadyDownloaded =
      window.electronAPI.onUpdateAlreadyDownloaded?.((data) => handleUpdateAlreadyDownloaded({ info: data })) ||
      (() => {});
    const unsubscribeClearChecking =
      window.electronAPI.onClearUpdateCheckingNotification?.(handleClearCheckingNotification) || (() => {});
    ...
    const unsubscribeError = window.electronAPI.onUpdateError((data) => handleUpdateError(data?.message ?? ''));
