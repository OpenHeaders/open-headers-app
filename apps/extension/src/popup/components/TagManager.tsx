...
const TagManager: React.FC = () => {
    ...
    for (const ruleId of ruleIds) {
      runtime.sendMessage({ type: 'toggleRule', ruleId, enabled }, (response: unknown) => {
        if (!(response as { success?: boolean })?.success)
          console.error(new Date().toISOString(), `ERROR`, `[TagManager]`, `Failed to toggle rule ${ruleId}`);
      });
    }
  ...
  const getGroupStats = (rules: RuleWithId[]): GroupStats => {
    ...
    return {
      ...
      requestRules: rules.filter((r) => !r.isResponse).length,
      responseRules: rules.filter((r) => r.isResponse).length,
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
        ...
        <Space onClick={(e: React.MouseEvent) => e.stopPropagation()}>
          <Tooltip
            title={
              isConnected ? `${stats.allEnabled ? 'Disable' : 'Enable'} all rules in this group` : 'App not connected'
            }
          >
            <Switch
              size="small"
        ...
        <Space style={{ width: '100%', justifyContent: 'space-between' }}>
          <Space>
            ...
            <div>
              ...
              <div>
                <Tag color={rule.isResponse ? 'blue' : 'green'} style={{ fontSize: '10px' }}>
        ...
        >
          {groupedRules.map(([groupKey, groupData]) => (
            <Panel
              ...
              style={{ marginBottom: '8px', border: '1px solid var(--border-color)', borderRadius: '4px' }}
            >
              <List
