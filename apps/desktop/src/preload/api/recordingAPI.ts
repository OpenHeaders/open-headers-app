...
const recordingAPI = {
  ...
  saveRecording: (recordData: RecordingData): Promise<{ success: boolean; recordId?: string }> =>
    ipcRenderer.invoke('saveRecording', recordData),
  saveUploadedRecording: (recordData: RecordingData): Promise<{ success: boolean; recordId?: string }> =>
    ipcRenderer.invoke('saveUploadedRecording', recordData),
  deleteRecording: (recordId: string): Promise<{ success: boolean }> => ipcRenderer.invoke('deleteRecording', recordId),
  downloadRecording: (record: RecordingMetadataEvent): Promise<{ success: boolean; error?: string }> =>
    ipcRenderer.invoke('downloadRecording', record),
  updateRecordingMetadata: (
    data: RecordingMetadataUpdateRequest,
  ): Promise<{ success: boolean; metadata?: RecordingMetadataEvent }> =>
    ipcRenderer.invoke('updateRecordingMetadata', data),
  ...
  onRecordingMetadataUpdated: (
    ...
    const subscription = (
      ...
      data: { recordId: string; updates: RecordingMetadataUpdateRequest['updates']; metadata?: RecordingMetadataEvent },
    ) => callback(data);
