...
describe('SparseCheckoutManager', () => {
  ...
  describe('validatePatterns()', () => {
    it('accepts valid directory patterns', () => {
      expect(() =>
        manager.validatePatterns(['/config/', '/.openheaders/workspaces/ws-prod/', '/src/services/']),
      ).not.toThrow();
    });
  ...
  describe('pathToPattern()', () => {
    it('returns directory pattern for nested config file', () => {
      expect(manager.pathToPattern('.openheaders/workspaces/ws-prod/sources.json')).toBe(
    ...
    it('handles deeply nested paths', () => {
      expect(manager.pathToPattern('.openheaders/workspaces/ws-a1b2c3d4/config/environments.json')).toBe(
        '/.openheaders/workspaces/ws-a1b2c3d4/config/',
      );
    });
  });
  ...
  describe('optimizePatternsForCone()', () => {
    ...
    it('deduplicates when multiple files resolve to same directory', () => {
      const result = manager.optimizePatternsForCone(['/config/sources.json', '/config/rules.json', '/config/']);
  ...
  describe('setPatterns()', () => {
    it('throws when patterns array is empty', async () => {
      await expect(manager.setPatterns('/repo', [])).rejects.toThrow('At least one pattern is required');
    });
  ...
  describe('removePatterns()', () => {
    ...
    it('throws when removing all patterns', async () => {
      ...
      await expect(manager.removePatterns('/repo', ['/config/'])).rejects.toThrow('Cannot remove all patterns');
    });
  });
