 ...
 * });
 */
export const createContentTypeHandler =
  ({ setContentType, form }: ContentTypeHandlerParams) =>
  (value: string) => {
...
export const createRefreshToggleHandler =
  ({ setRefreshEnabled, refreshEnabledRef, customIntervalRef, refreshTypeRef, form }: RefreshToggleParams) =>
  (checked: boolean) => {
...
export const createJsonFilterToggleHandler =
  ...
  (enabled: boolean) => {
    ...
    const currentJsonFilter = form.getFieldValue('jsonFilter') || { enabled: false, path: '' };
