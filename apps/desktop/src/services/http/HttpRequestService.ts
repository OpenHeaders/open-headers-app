  ...
  async execute(spec: HttpRequestSpec): Promise<HttpRequestResult> {
    ...
    if (spec.totpSecret) {
      ...
      if (cooldown.inCooldown) {
        throw new Error(
          `TOTP cooldown active. Please wait ${cooldown.remainingSeconds} seconds before making another request.`,
        );
      }
    ...
    if (!url || url === 'https://' || url === 'http://') {
      throw new Error(
        `Invalid URL after variable substitution: "${url}". Check that all environment variables are defined.`,
      );
    }
    ...
    if (spec.headers) {
      for (const h of spec.headers) {
        if (h.key) {
          headers[this.resolveString(h.key, envVars, totpCode)] = this.resolveString(h.value, envVars, totpCode);
        }
      }
    }
    ...
    log.debug(
      `Request details — Content-Type: ${contentType}, headers: ${JSON.stringify(Object.keys(headers))}, bodyLength: ${body?.length ?? 0}, hasTOTP: ${!!totpCode}`,
    );
...
export { encodeFormBody, HttpRequestService };
