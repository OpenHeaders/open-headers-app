...
const ConnectionProgressModal = ({
    ...
    if (connectionProgress && connectionProgress.length > 0) {
      return (
        <div style={{ marginBottom: 16 }}>
          <div style={{ marginBottom: 16 }}>
            <strong>{isTestingConnection ? 'Testing Git Connection...' : 'Connection Test Results'}</strong>
          </div>
          ...
          >
            {connectionProgress.map((step, index) => (
              <div key={step.step || index} style={{ marginBottom: 12 }}>
                <div style={{ fontSize: '14px', marginBottom: 4 }}>{step.step || 'Processing...'}</div>
                {step.details && (
                  <div style={{ fontSize: '12px', color: '#666', marginBottom: 4 }}>{String(step.details)}</div>
                )}
              ...
              percent={
                connectionProgress.length > 0
                  ? // Use specific progress if available, otherwise fallback to step-based calculation
                    ...
                    : Math.min(90, connectionProgress.length * 12)
                  : 10
              }
              status="active"
              showInfo={false}
  ...
  const renderFooter = () => {
    ...
    buttons.push(
      <Button key="close" onClick={handleClose} icon={<CloseOutlined />} disabled={false}>
        Close
      </Button>,
    );
