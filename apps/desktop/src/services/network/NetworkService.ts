...
class NetworkService extends EventEmitter {
  ...
  startAdaptiveMonitoring(): void {
    const scheduleNextCheck = (): void => {
      const interval = this.isNetworkStable ? this.adaptiveCheckInterval : this.checkInterval;

      this.intervals.set(
        'networkCheck',
        setTimeout(() => {
          if (!this.isDestroyed) {
            this.checkNetworkState().catch((err) => this.log.error('Network state check error:', err));
            scheduleNextCheck();
          }
        }, interval),
      );
    ...
    const scheduleNextQuickCheck = (): void => {
      const interval = this.isNetworkStable ? this.adaptiveQuickCheckInterval : this.quickCheckInterval;

      this.intervals.set(
        'quickCheck',
        setTimeout(() => {
          if (!this.isDestroyed) {
            this.quickConnectivityCheck().catch((err) => this.log.error('Quick connectivity check error:', err));
  ...
  updateState(changes: Partial<NetworkState>, immediate = false): void {
    if (Object.hasOwn(changes, 'isOnline')) {
      if (!changes.isOnline) {
  ...
  applyStateWithHysteresis(newState: Partial<NetworkState>): void {
    ...
    if (currentIsOnline !== newIsOnline) {
      if (this.hysteresisTimeout) {
        this.log.debug(
          `State change blocked by hysteresis (current: ${currentIsOnline ? 'online' : 'offline'}, attempted: ${newIsOnline ? 'online' : 'offline'})`,
        );
        return;
      }
      ...
      if (!this.isStableStateChange()) {
        this.log.debug(
          `State change blocked - not stable (current: ${currentIsOnline ? 'online' : 'offline'}, attempted: ${newIsOnline ? 'online' : 'offline'})`,
        );
        return;
      }
      ...
      this.log.info(
        `Applying state change with hysteresis: ${currentIsOnline ? 'online' : 'offline'} -> ${newIsOnline ? 'online' : 'offline'}`,
      );
    }
  ...
  async quickConnectivityCheck(): Promise<void> {
    ...
    try {
      ...
      if (result.success !== this.state.isOnline) {
        ...
        if (
          (result.success && this.consecutiveOnlineChecks >= this.requiredConsecutiveChecks) ||
          (!result.success && this.consecutiveOfflineChecks >= this.requiredConsecutiveChecks)
        ) {
          this.log.warn(
            `QUICK CHECK STATE CHANGE: ${this.state.isOnline ? 'online' : 'offline'} -> ${result.success ? 'online' : 'offline'}`,
            {
          ...
          this.log.debug(
            `Quick check detected potential change but waiting for confirmation (${result.success ? 'online' : 'offline'} count: ${result.success ? this.consecutiveOnlineChecks : this.consecutiveOfflineChecks})`,
          );
        }
      } else {
        this.consecutiveOnlineChecks = 0;
  ...
  async getNetworkInterfaces(): Promise<Map<string, NetworkInterfaceInfo>> {
    ...
    for (const [name, addresses] of Object.entries(netInterfaces)) {
      ...
      const validAddresses = addresses.filter((addr) => !addr.internal && addr.family === 'IPv4');
  ...
  async checkInternetConnectivity(): Promise<EndpointResult[]> {
    const checkPromises = this.connectivityEndpoints.map((endpoint) => this.checkEndpoint(endpoint));
