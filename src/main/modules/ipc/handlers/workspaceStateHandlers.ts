...
export function registerWorkspaceStateHandlers(): void {
    ...
    ipcMain.handle('workspace-state:create-workspace', async (_event, workspace: Partial<Workspace> & { id: string; name: string; type: WorkspaceType }) => {
        try {
    ...
    ipcMain.handle('workspace-state:update-workspace', async (_event, workspaceId: string, updates: Partial<Workspace>) => {
        try {
