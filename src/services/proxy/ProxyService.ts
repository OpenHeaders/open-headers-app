...
class ProxyService extends EventEmitter {
    ...
    handleProxyResponse(proxyRes: http.IncomingMessage, req: http.IncomingMessage, res: http.ServerResponse, targetUrl: string, requestId: string): void {
        ...
        proxyRes.on('end', async () => {
            ...
            if (urlLower.endsWith('.js') || urlLower.endsWith('.mjs')) {
                ...
                for (const [ext, type] of Object.entries(contentTypeMap)) {
                    if (urlLower.endsWith(ext)) { contentType = type; break; }
                }
            }
            ...
            if (req.method === 'GET' && (proxyRes.statusCode || 0) >= 200 && (proxyRes.statusCode || 0) < 300 && this.cacheEnabled) {
    ...
    addTrustedCertificate(fingerprint: string): void { this.trustedCertificates.add(fingerprint); }
    removeTrustedCertificate(fingerprint: string): void { this.trustedCertificates.delete(fingerprint); }
    ...
    removeCertificateException(domain: string): void { this.certificateExceptions.delete(domain); }
    ...
    async getCacheStats() { return await this.cache.getStats(); }
    async getCacheEntries() { return await this.cache.getCacheEntries(); }
