...
describe('FileHandlers', () => {
  ...
  describe('handleReadFile', () => {
    it('reads file with utf8 encoding by default', async () => {
      ...
      expect(mockFsReadFile).toHaveBeenCalledWith('/Users/jane.doe/Documents/OpenHeaders/tokens/staging.json', 'utf8');
    });
    ...
    it('throws on file not found', async () => {
      ...
      await expect(handlers.handleReadFile(mockEvent, '/nonexistent/path/file.txt')).rejects.toThrow('ENOENT');
    });

    it('throws on permission denied', async () => {
      mockFsReadFile.mockRejectedValueOnce(new Error('EACCES: permission denied'));

      await expect(handlers.handleReadFile(mockEvent, '/protected/file.txt')).rejects.toThrow('EACCES');
    });
  });
  ...
  describe('handleWriteFile', () => {
    it('writes JSON file atomically', async () => {
      ...
      await handlers.handleWriteFile(mockEvent, '/Users/jane.doe/Documents/OpenHeaders/config.json', content);
  ...
  describe('handleWatchFile', () => {
    ...
    it('throws when file does not exist', async () => {
      ...
      await expect(handlers.handleWatchFile(mockEvent, 'src-1', '/nonexistent/file.txt')).rejects.toThrow('ENOENT');
    });
  });
  ...
  describe('handleGetEnvVariable', () => {
    ...
    it('returns descriptive message for unset variable', () => {
      const result = handlers.handleGetEnvVariable(mockEvent, 'OPENHEADERS_NONEXISTENT_VAR_a1b2c3d4');
  ...
  describe('handleOpenRecordFile', () => {
    ...
    it('throws when file does not exist', async () => {
      ...
      await expect(handlers.handleOpenRecordFile(mockEvent, '/nonexistent/record.json')).rejects.toThrow('ENOENT');
    });
  });
  ...
  describe('handleGetResourcePath', () => {
    ...
    it('throws when resource is not found anywhere', async () => {
      ...
      await expect(handlers.handleGetResourcePath(mockEvent, 'nonexistent-resource.txt')).rejects.toThrow(
        'Resource not found: nonexistent-resource.txt',
      );
    });
  });
});
