...
class SourceRefreshService {
    ...
    if (!wasEnabled && isEnabled) {
      ...
      if (oldInterval !== newInterval) {
        ...
        if (schedule && this.timers.has(sourceId)) {
          ...
          log.info(
            `Source ${sourceId} interval changed to ${newInterval}m — active timer preserved, new interval applies after next refresh`,
          );
        } else {
  ...
  async refreshSource(
    sourceId: string,
    options: { reason?: string; bypassCircuitBreaker?: boolean } = {},
  ...
  private async scheduleSource(source: Source): Promise<void> {
    ...
    const schedule: ScheduleEntry = {
      ...
      lastRefresh:
        existing?.lastRefresh ?? source.refreshOptions?.lastRefresh ?? source.refreshStatus?.lastRefresh ?? null,
      nextRefresh: null,
      failureCount: 0,
    };
  ...
  private async _calculateAndScheduleNextAsync(sourceId: string): Promise<void> {
    ...
    if (cbStatus.failureCount > 0 || breaker.isOpen()) {
      if (breaker.isOpen()) {
        ...
        nextRefreshTime =
          now + calculateDelayWithJitter(INITIAL_RETRY_CONFIG.baseDelay, INITIAL_RETRY_CONFIG.maxJitter);
      }
