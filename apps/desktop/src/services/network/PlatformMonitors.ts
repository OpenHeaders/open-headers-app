...
class MacOSNetworkMonitor extends BasePlatformMonitor {
    ...
    const checkVPN = async (): Promise<void> => {
      try {
        ...
        if (initialCheckDone && (lastVPNState !== activeVPN || lastVPNInterface !== vpnName)) {
          this.log.info(
            `VPN state changed: ${activeVPN ? 'connected' : 'disconnected'} ${vpnName ? `(${vpnName})` : ''}`,
          );
          this.emit('vpn-state', {
            active: activeVPN,
            name: vpnName,
  ...
  async checkVPNInterfaces(): Promise<void> {
    try {
      ...
      const vpnPatterns = [/utun\d+:.*UP/, /tun\d+:.*UP/, /tap\d+:.*UP/, /ppp\d+:.*UP/, /ipsec\d+:.*UP/];
  ...
  watchVPNState(): void {
    ...
    const checkVPN = async (): Promise<void> => {
      ...
      try {
        ...
        try {
          const vpnOutput = await this.executeCommand(
            'powershell',
            [
              '-NoProfile',
              '-NonInteractive',
        ...
        if (!activeVPN) {
          try {
            const adapterOutput = await this.executeCommand(
              'powershell',
              [
                '-NoProfile',
                '-NonInteractive',
...
export { BasePlatformMonitor, GenericNetworkMonitor, LinuxNetworkMonitor, MacOSNetworkMonitor, WindowsNetworkMonitor };
