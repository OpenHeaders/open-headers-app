...
export class RecordingStateMachine {
  ...
  constructor() {
    ...
    this.transitions = [
      ...
      {
        from: [RecordingStates.RECORDING, RecordingStates.PRE_NAVIGATION],
        to: RecordingStates.STOPPING,
        event: 'STOP_RECORDING',
      },
      { from: [RecordingStates.STOPPING], to: RecordingStates.IDLE, event: 'RECORDING_STOPPED' },
      {
        from: [RecordingStates.STARTING, RecordingStates.RECORDING, RecordingStates.STOPPING],
        to: RecordingStates.ERROR,
        event: 'ERROR',
      },
  ...
  tryTransition(tabId: number, event: TransitionEvent, data?: { error?: string } | null): RecordingStateValue | null {
    ...
    const transition = this.transitions.find((t) => t.event === event && t.from.includes(currentState.state));
  ...
  getRecordingTabs(): number[] {
    ...
    for (const [tabId, state] of this.tabStates) {
      if (state.state === RecordingStates.RECORDING || state.state === RecordingStates.PRE_NAVIGATION) {
