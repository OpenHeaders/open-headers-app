...
describe('CommitManager', () => {
    ...
    describe('escapeMessage()', () => {
        it('escapes double quotes', () => {
            expect(manager.escapeMessage('feat: Add "OAuth2 Bearer Token" source'))
                .toBe('feat: Add \\"OAuth2 Bearer Token\\" source');
        });

        it('escapes dollar signs', () => {
            expect(manager.escapeMessage('fix: Handle $TOKEN variable'))
                .toBe('fix: Handle \\$TOKEN variable');
        });

        it('escapes both quotes and dollar signs', () => {
            expect(manager.escapeMessage('Set "$API_KEY" to new value'))
                .toBe('Set \\"\\$API_KEY\\" to new value');
        });

        it('returns unchanged string without special chars', () => {
            expect(manager.escapeMessage('chore: Update workspace configuration'))
                .toBe('chore: Update workspace configuration');
        });
    });
    ...
    describe('createCommit()', () => {
        ...
        it('throws specific error when nothing to commit', async () => {
            ...
            await expect(manager.createCommit('/repo', 'test'))
    ...
    describe('stageFile()', () => {
        ...
        it('handles files with spaces in path', async () => {
            ...
            expect(executeSpy).toHaveBeenCalledWith(
                'add "OpenHeaders Config/staging tokens.json"',
                { cwd: '/repo' }
            );
        });
    });
    ...
    describe('getHistory()', () => {
        ...
        it('defaults to 10 commits', async () => {
            ...
            expect(executeSpy).toHaveBeenCalledWith(
                'log -10 --pretty=format:"%H|%an|%ae|%at|%s"',
                { cwd: '/repo' }
            );
        });
    });
    ...
    describe('commitConfiguration()', () => {
        it('throws when repoDir is missing', async () => {
            await expect(
                manager.commitConfiguration({ repoDir: '' })
            ).rejects.toThrow('Repository directory is required');
        });

        it('throws when neither files nor configPaths are provided', async () => {
            await expect(
                manager.commitConfiguration({ repoDir: '/repo' })
    ...
    describe('ensureGitUser()', () => {
        ...
        it('uses provided author and email when not configured', async () => {
            ...
            expect(executeSpy).toHaveBeenCalledWith(
                'config user.email "jane.doe@openheaders.io"',
                { cwd: '/repo' }
            );
        });
    });
