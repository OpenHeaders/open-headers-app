...
import { HttpRequestService, encodeFormBody } from '../../../../src/services/http/HttpRequestService';
    ...
    describe('execute', () => {
        ...
        it('resolves {{VAR}} in body', async () => {
            ...
            expect(request.write).toHaveBeenCalledWith(
                Buffer.from('{"user":"admin@openheaders.io"}')
            );
        });
        ...
        it('throws on TOTP cooldown', async () => {
            ...
            ).rejects.toThrow('TOTP cooldown active');
        });
        ...
        it('throws on timeout', async () => {
            ...
            await expect(
                service.execute(makeSpec({ timeout: 100 }))
