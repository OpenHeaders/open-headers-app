...
interface UnifiedHeaderModalProps {
  visible: boolean;
  onCancel: () => void;
  onSave: (values: HeaderRuleFormValues) => void;
  initialValues: HeaderRuleValues | null;
}
const UnifiedHeaderModal = ({ visible, onCancel, onSave, initialValues }: UnifiedHeaderModalProps) => {
  ...
  const domainValidation = useMemo((): DomainValidation[] => {
    ...
    return domainsValue.map((domain: string) => validateEnvironmentVariables(domain, variables));
      ...
      if (initialValues) {
        if (currentMode === 'cookie') {
          ...
          form.setFieldsValue({
            ...
            expires:
              parsed.expires || initialValues.expires
                ? dayjs(parsed.expires || (initialValues.expires as string | undefined))
                : undefined,
          });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ValueSection mode={mode} valueType={valueType} setValueType={setValueType} envContext={envContext} />
        ...
        {/* Domains section */}
        <DomainSection domainValidation={domainValidation} envContext={envContext} />

        {/* Environment Variable Info */}
        <EnvVarInfo envVarValidation={envVarValidation} mode={mode} />
        ...
        >
          <Space style={{ width: '100%', justifyContent: 'center' }}>
            <Button onClick={onCancel} icon={<CloseOutlined />} size="small" style={{ minWidth: 100 }}>
              Cancel
            </Button>

            <Button type="primary" htmlType="submit" icon={<SaveOutlined />} size="small" style={{ minWidth: 100 }}>
