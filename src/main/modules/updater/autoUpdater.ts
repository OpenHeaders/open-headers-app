...
class AutoUpdaterManager {
     ...
     */
    private classifyUpdateError(err: Error): { kind: 'network' | 'transient' | 'signature' | 'generic'; userMessage: string } {
        ...
        if (msg.includes('code signature') || msg.includes('signature verification')) {
            log.error('Code signature validation error:', { message: msg, code: toErrno(err).code, errno: toErrno(err).errno });
