...
export const TotpProvider = ({ children }) => {
    ...
    const checkIfRequestUsesTotp = useCallback((url, method, requestOptions) => {
        // Check if _TOTP_CODE is present anywhere
        const checkString = (str) => str && str.includes('_TOTP_CODE');
        ...
        // Check body
        if (requestOptions?.body && checkString(requestOptions.body)) return true;
    ...
    const canUseTotpForSource = useCallback((sourceId) => {
        ...
        if (testingSourcesRef.current.has(sourceId)) {
            return false;
        }
