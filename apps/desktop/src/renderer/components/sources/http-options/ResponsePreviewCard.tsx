...
interface ResponsePreviewCardProps {
  testResponseVisible: boolean;
  testResponseContent: TestResponseContent | string | null;
}
  ...
  return (
      ...
      title={
        <div className="response-card-header">
          ...
          {typeof testResponseContent === 'object' &&
            ...
            testResponseContent.statusCode && (
              <span className="status-display">
                Status Code{' '}
                <span className={`status-code status-${Math.floor(testResponseContent.statusCode / 100)}xx`}>
                  {testResponseContent.statusCode} {getStatusText(testResponseContent.statusCode)}
                </span>
                {testResponseContent.duration && (
                  <span className="duration-display">• {testResponseContent.duration}ms</span>
                )}
              </span>
            )}
        </div>
    ...
    >
      {typeof testResponseContent === 'object' && testResponseContent !== null ? (
            ...
            <Tabs
              ...
              items={[
                      ...
                      {(() => {
                        try {
                          ...
                          if (content.filteredWith) {
                            return (
                              <div>
                                <div className="filter-info">[Filtered with path: {content.filteredWith}]</div>
                ...
                },
                {
                  key: 'raw',
                  label: 'Raw',
                  children: <pre className="response-raw">{JSON.stringify(content, null, 2)}</pre>,
                },
              ]}
            />
          );
        ...
        <pre className="response-error">{String(testResponseContent ?? '')}</pre>
      )}
    </Card>
  );
};
