...
describe('TokenAuthStrategy', () => {
  ...
  describe('getAuthUrl()', () => {
    it('GitHub: uses token as username with x-oauth-basic password', () => {
      const url = strategy.getAuthUrl('https://github.com/OpenHeaders/open-headers-app.git', GITHUB_PAT, 'github');
    ...
    it('generic: uses token as password with "token" username', () => {
      const url = strategy.getAuthUrl('https://gitea.openheaders.io/team/repo.git', GENERIC_TOKEN, 'generic');
      const parsed = new URL(url);
      expect(parsed.username).toBe('token');
    ...
    it('auto: detects token type from GitHub hostname', () => {
      const url = strategy.getAuthUrl('https://github.com/OpenHeaders/open-headers-app.git', GITHUB_PAT, 'auto');
      const parsed = new URL(url);
      expect(parsed.username).toBe(GITHUB_PAT);
      expect(parsed.password).toBe('x-oauth-basic');
    });
    ...
    it('auto: detects token type from GitLab hostname', () => {
      const url = strategy.getAuthUrl('https://gitlab.com/OpenHeaders/open-headers-app.git', GITLAB_TOKEN, 'auto');
      const parsed = new URL(url);
      expect(parsed.username).toBe('oauth2');
      expect(parsed.password).toBe(GITLAB_TOKEN);
    });
    ...
    it('throws for malformed URL', () => {
      expect(() => strategy.getAuthUrl('not-a-valid-url', GITHUB_PAT, 'github')).toThrow('Failed to parse Git URL');
    });

    it('throws for empty URL', () => {
      expect(() => strategy.getAuthUrl('', GITHUB_PAT, 'github')).toThrow('Failed to parse Git URL');
    });

    it('defaults tokenType to auto when omitted', () => {
      const url = strategy.getAuthUrl('https://github.com/OpenHeaders/open-headers-app.git', GITHUB_PAT);
      const parsed = new URL(url);
      expect(parsed.username).toBe(GITHUB_PAT);
      expect(parsed.password).toBe('x-oauth-basic');
    });
  ...
  describe('setup()', () => {
    ...
    it('defaults tokenType to auto when not specified', async () => {
      const result = await strategy.setup('https://github.com/OpenHeaders/open-headers-app.git', { token: GITHUB_PAT });
