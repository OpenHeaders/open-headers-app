...
describe('ErrorRecovery', () => {
  ...
  describe('classifyError()', () => {
    ...
    it('classifies "network" in message as NETWORK', () => {
      expect(
        recovery.classifyError(new Error('Network request failed for https://auth.openheaders.io/oauth2/token')),
      ).toBe(ErrorTypes.NETWORK);
    });

    it('classifies "offline" in message as NETWORK', () => {
      expect(recovery.classifyError(new Error('Device is offline — cannot sync workspace'))).toBe(ErrorTypes.NETWORK);
    });
    ...
    it('classifies "unauthorized" in message as AUTH', () => {
      expect(recovery.classifyError(new Error('Request unauthorized for gitlab.openheaders.io'))).toBe(ErrorTypes.AUTH);
    });

    it('classifies "forbidden" in message as AUTH', () => {
      expect(recovery.classifyError(new Error('Access forbidden to private repository'))).toBe(ErrorTypes.AUTH);
    });

    it('classifies "authentication" in message as AUTH', () => {
      expect(recovery.classifyError(new Error('Authentication required for workspace sync'))).toBe(ErrorTypes.AUTH);
    });

    it('classifies "permission" in message as AUTH', () => {
      expect(recovery.classifyError(new Error('Permission denied writing to team workspace config'))).toBe(
        ErrorTypes.AUTH,
      );
    });
    ...
    it('classifies "timed out" in message as TIMEOUT', () => {
      expect(recovery.classifyError(new Error('Request timed out after 30000ms for source refresh'))).toBe(
        ErrorTypes.TIMEOUT,
      );
    });
    ...
    it('classifies "rate limit" in message as RATE_LIMIT', () => {
      expect(recovery.classifyError(new Error('Rate limit exceeded for api.openheaders.io'))).toBe(
        ErrorTypes.RATE_LIMIT,
      );
    });

    it('classifies "too many requests" in message as RATE_LIMIT', () => {
      expect(recovery.classifyError(new Error('Too many requests — retry after 60s'))).toBe(ErrorTypes.RATE_LIMIT);
    });
    ...
    it('classifies "already exists" in message as CONFLICT', () => {
      expect(recovery.classifyError(new Error('Workspace "Production" already exists'))).toBe(ErrorTypes.CONFLICT);
    });

    // Resource exhausted
    it('classifies "memory" in message as RESOURCE_EXHAUSTED', () => {
      expect(recovery.classifyError(new Error('JavaScript heap out of memory during recording export'))).toBe(
        ErrorTypes.RESOURCE_EXHAUSTED,
      );
    });

    it('classifies "disk space" in message as RESOURCE_EXHAUSTED', () => {
      expect(recovery.classifyError(new Error('Not enough disk space to save recording'))).toBe(
        ErrorTypes.RESOURCE_EXHAUSTED,
      );
    });

    it('classifies "quota" in message as RESOURCE_EXHAUSTED', () => {
      expect(recovery.classifyError(new Error('Storage quota exceeded for workspace data'))).toBe(
        ErrorTypes.RESOURCE_EXHAUSTED,
      );
    });
    ...
    it('classifies unknown errors as UNKNOWN', () => {
      expect(recovery.classifyError(new Error('Something completely unexpected happened'))).toBe(ErrorTypes.UNKNOWN);
    });
  });
