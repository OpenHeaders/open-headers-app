...
describe('StorageHandlers', () => {
  ...
  describe('handleSaveToStorage', () => {
    ...
    it('saves non-JSON file atomically with writeFile', async () => {
      ...
      expect(mockAtomicWriteFile).toHaveBeenCalledWith(expect.stringContaining('token.txt'), content);
    });
    ...
    it('falls back to writeFile for invalid JSON in .json file', async () => {
      ...
      expect(mockAtomicWriteFile).toHaveBeenCalledWith(expect.stringContaining('corrupt.json'), invalidJson);
    });
    ...
    it('throws on write error', async () => {
      ...
      await expect(handlers.handleSaveToStorage(mockEvent, 'data.txt', 'content')).rejects.toThrow('ENOSPC');
    });
  });
  ...
  describe('handleLoadFromStorage', () => {
    ...
    it('throws on non-ENOENT errors', async () => {
      ...
      await expect(handlers.handleLoadFromStorage(mockEvent, 'protected.json')).rejects.toThrow('EACCES');
    });
  });
  ...
  describe('handleDeleteFromStorage', () => {
    it('deletes existing file and returns true', async () => {
      ...
      expect(mockFsUnlink).toHaveBeenCalledWith(expect.stringContaining('old-data.json'));
    });
    ...
    it('throws on non-ENOENT errors', async () => {
      ...
      await expect(handlers.handleDeleteFromStorage(mockEvent, 'protected.json')).rejects.toThrow('EACCES');
    });
  });
  ...
  describe('handleDeleteDirectory', () => {
    it('deletes existing directory recursively', async () => {
      ...
      const result = await handlers.handleDeleteDirectory(mockEvent, 'workspaces/a1b2c3d4-e5f6-7890-abcd-ef1234567890');
      expect(result).toBe(true);
    ...
    it('throws on non-ENOENT errors', async () => {
      ...
      await expect(handlers.handleDeleteDirectory(mockEvent, 'protected-dir')).rejects.toThrow('EACCES');
    });
    ...
    it('uses correct path under userData directory', async () => {
      ...
      expect(mockFsRm).toHaveBeenCalledWith('/tmp/open-headers-test/userData/recordings/rec-a1b2c3d4', {
        recursive: true,
        force: true,
      });
    });
