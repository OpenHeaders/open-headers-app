...
vi.mock('../../../../src/utils/atomicFileWriter.js', () => ({
    default: { writeJson: (...args: unknown[]) => mockWriteJson(...args), readJson: (...args: unknown[]) => mockReadJson(...args) }
}));
...
describe('WorkspaceStateService.onCliWorkspaceCreated', () => {
    ...
    it('imports synced data via syncScheduler.importSyncedData', async () => {
        ...
        expect(syncScheduler.importSyncedData).toHaveBeenCalledWith(
            workspaceId,
            syncData,
    ...
    it('starts auto-sync with skipInitialSync: true', async () => {
        ...
        expect(syncScheduler.onWorkspaceSwitch).toHaveBeenCalledWith(
            workspaceId,
    ...
    it('persists workspaces config to disk', async () => {
        ...
        const workspacesJsonWrite = mockWriteJson.mock.calls.find(
            (c: unknown[]) => (c[0] as string).includes('workspaces.json')
        );
    ...
    it('processes complete flow in correct order', async () => {
        ...
        // Track call order
        syncScheduler.importSyncedData.mockImplementation(async () => { callOrder.push('importSyncedData'); });
        syncScheduler.onWorkspaceSwitch.mockImplementation(async () => { callOrder.push('onWorkspaceSwitch'); });
