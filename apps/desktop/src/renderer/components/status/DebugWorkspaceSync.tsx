                ...
                <div style={{
                    ...
                    <div style={{ marginTop: 10 }}>
                        {teamWorkspaces.length === 0 && (
                            <div style={{ color: '#999' }}>No syncable (git) workspaces</div>
                        )}
                        {teamWorkspaces.map(({ workspace, status }) => {
                            ...
                            const statusColor = isSyncing ? '#40a9ff' :
                                hasError ? '#ff6b6b' :
                                status.lastSync ? '#52c41a' : '#999';

                            const statusLabel = isSyncing ? 'SYNCING' :
                                hasError ? 'ERROR' :
                                status.lastSync ? 'OK' : 'PENDING';
                            ...
                            return (
                                <div key={workspace.id} style={{
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px 16px', color: '#ccc' }}>
                                        <div>ID: <span style={{ color: '#fff' }}>{workspace.id.substring(0, 20)}...</span></div>
                                        <div>Type: <span style={{ color: '#fff' }}>{workspace.type}</span></div>
                                        <div>Auto-sync: <span style={{ color: '#fff' }}>{workspace.autoSync !== false ? 'Yes' : 'No'}</span></div>
                                        <div>Branch: <span style={{ color: '#fff' }}>{workspace.gitBranch || 'main'}</span></div>
                                        <div>Last Sync: <span style={{ color: '#fff' }}>{formatTimeAgo(status.lastSync)}</span></div>
                                        <div>At: <span style={{ color: '#fff' }}>{formatTimestamp(status.lastSync)}</span></div>
                                        {workspace.metadata && (
                                            <>
                                                <div>Sources: <span style={{ color: '#fff' }}>{workspace.metadata.sourceCount ?? '—'}</span></div>
                                                <div>Rules: <span style={{ color: '#fff' }}>{workspace.metadata.ruleCount ?? '—'}</span></div>
                                                <div>Proxy Rules: <span style={{ color: '#fff' }}>{workspace.metadata.proxyRuleCount ?? '—'}</span></div>
                                                <div>Data Load: <span style={{ color: '#fff' }}>{formatTimestamp(workspace.metadata.lastDataLoad)}</span></div>
                                            </>
                                        )}
                                        {status.commitInfo && (
                                            <>
                                                <div>Commit: <span style={{ color: '#fff' }}>{status.lastCommit?.substring(0, 7) || '—'}</span></div>
                                                <div>Author: <span style={{ color: '#fff' }}>{status.commitInfo.author || '—'}</span></div>
                                            </>
                                        )}
