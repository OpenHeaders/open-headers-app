...
class CliApiService {
  ...
  _loadLogs(): void {
    try {
      ...
      for (const line of lines) {
        try {
          this.requestLogs.push(JSON.parse(line) as RequestLogEntry);
        } catch {
          /* skip */
        }
      }
  ...
  _lookupClientProcess(remotePort: number): string | null {
    try {
      switch (process.platform) {
        case 'darwin':
          return this._lookupLsof(remotePort);
        case 'linux':
          return this._lookupSs(remotePort);
        case 'win32':
          return this._lookupNetstat(remotePort);
        default:
          return null;
      }
    } catch {
  ...
  _lookupLsof(remotePort: number): string | null {
    ...
    const stdout = execFileSync(
      'lsof',
      ['-nP', '-i', `TCP:${this.port}`, '-sTCP:ESTABLISHED', '-Fp', '-Fc', '-Fn'],
      execOpts,
    );
  ...
  _lookupNetstat(remotePort: number): string | null {
    ...
    for (const line of stdout.split('\n')) {
      ...
      const addr1 = parts[1],
        addr2 = parts[2];
      const match =
        (addr1.endsWith(serverPort) && addr2.endsWith(clientPort)) ||
  ...
  _handleWithBody(
    ...
    handler: (body: JsonValue) => Promise<void>,
  ): void {
    let body = '';
  ...
  async _handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    res.writeHead = function (
      ...
      statusCode: number,
      ...rest: Parameters<typeof origWriteHead> extends [number, ...infer R] ? R : never[]
    ) {
    ...
    res.end = function (
      ...
      ...rest: Parameters<typeof origEnd> extends [infer _, ...infer R] ? R : never[]
    ) {
