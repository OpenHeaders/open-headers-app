...
class ConcurrentMap {
  ...
  /**
   * Execute a function with exclusive access to the map
   */
  async withLock(fn) {
    return this.mutex.withLock(() => fn(this.map));
  }
}
...
class ConcurrentSet {
  ...
  /**
   * Execute a function with exclusive access to the set
   */
  async withLock(fn) {
    return this.mutex.withLock(() => fn(this.set));
  }
}
...
class RequestDeduplicator {
   ...
   */
  async isPending(key) {
    return this.mutex.withLock(() => this.pendingRequests.has(key));
  }
