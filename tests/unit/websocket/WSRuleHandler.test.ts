...
function createMockService(rules: any = {}, sources: any[] = []) {
    return {
        rules,
        sources,
        appDataPath: null,
...
describe('WSRuleHandler', () => {
    ...
    beforeEach(() => {
        ...
        handler = new WSRuleHandler(mockService as any);
    });

    // ------- _populateDynamicHeaderValues -------
    describe('_populateDynamicHeaderValues', () => {
        it('returns empty rules unchanged', () => {
            const result = handler._populateDynamicHeaderValues({});
            expect(result).toEqual({});
        });

        it('returns rules without header array unchanged', () => {
            const result = handler._populateDynamicHeaderValues({ other: 'data' } as any);
        ...
        it('passes through rules without env vars or dynamic sources', () => {
            const rules = {
                header: [
                    { id: '1', headerName: 'X-Custom', headerValue: 'value', isEnabled: true }
                ]
            };
        ...
        it('resolves dynamic header value from source', () => {
            mockService.sources = [
                { sourceId: '42', sourceContent: 'token-abc' }
            ];
            const rules = {
                header: [
                    { id: '1', isDynamic: true, sourceId: 42, prefix: 'Bearer ', suffix: '', headerName: 'Authorization' }
                ]
            };
            const result = handler._populateDynamicHeaderValues(rules);
        ...
        it('handles dynamic rule with prefix and suffix', () => {
            mockService.sources = [
                { sourceId: '1', sourceContent: 'XYZ' }
            ];
            const rules = {
                header: [
                    { id: '1', isDynamic: true, sourceId: '1', prefix: 'pre-', suffix: '-post', headerName: 'X-Token' }
                ]
            };
            const result = handler._populateDynamicHeaderValues(rules);
        ...
        it('leaves dynamic value empty when source not found', () => {
            ...
            const rules = {
                header: [
                    { id: '1', isDynamic: true, sourceId: '999', prefix: '', suffix: '', headerName: 'X-Token' }
                ]
            };
            const result = handler._populateDynamicHeaderValues(rules);
            expect(result.header![0].headerValue).toBeUndefined();
        });
        ...
        it('resolves env vars in header values', () => {
            ...
            const rules = {
                header: [
                    { id: '1', headerName: 'X-Key', headerValue: '{{API_KEY}}', hasEnvVars: true, envVars: ['API_KEY'] }
                ]
            };
            const result = handler._populateDynamicHeaderValues(rules);
        ...
        it('resolves env vars in header name', () => {
            ...
            const rules = {
                header: [
                    { id: '1', headerName: '{{HEADER}}', headerValue: 'val', hasEnvVars: true, envVars: ['HEADER'] }
                ]
            };
            const result = handler._populateDynamicHeaderValues(rules);
            expect(result.header![0].headerName).toBe('X-Custom');
        ...
        it('filters out rules with missing env var dependencies', () => {
            ...
            const rules = {
                header: [
                    { id: '1', headerName: 'X-Key', headerValue: '{{MISSING}}', hasEnvVars: true, envVars: ['MISSING'] }
                ]
            };
            const result = handler._populateDynamicHeaderValues(rules);
        ...
        it('resolves env vars in domains with comma splitting', () => {
            ...
            const rules = {
                header: [
                    { id: '1', headerName: 'X-Key', headerValue: 'val', hasEnvVars: true, envVars: ['DOMAINS'], domains: ['{{DOMAINS}}'] }
                ]
            };
            const result = handler._populateDynamicHeaderValues(rules);
        ...
        it('resolves env vars in dynamic prefix/suffix', () => {
            ...
            mockService.sources = [{ sourceId: '1', sourceContent: 'token' }];
            const rules = {
                header: [{
        ...
        it('does not modify original rules object', () => {
            const original = {
                header: [{ id: '1', headerName: 'X-Test', headerValue: 'original' }]
            };
    ...
    describe('updateRules', () => {
        it('sets rules on service and broadcasts', () => {
            ...
            handler.updateRules({ header: [{ id: '1' }] } as any);
            expect(mockService.rules).toEqual({ header: [{ id: '1' }] });
