...
const workspaceAPI = {
  ...
  initializeWorkspaceSync: (workspaceId: string): Promise<OperationResult> =>
    ipcRenderer.invoke('initializeWorkspaceSync', workspaceId),
  deleteWorkspaceFolder: (workspaceId: string): Promise<OperationResult> =>
    ipcRenderer.invoke('deleteWorkspaceFolder', workspaceId),
  syncWorkspace: (workspaceId: string, options: WorkspaceSyncOptions): Promise<OperationResult> =>
    ipcRenderer.invoke('workspace-sync', workspaceId, options),

  // Workspace management operations
  workspaceTestConnection: (gitConfig: GitTestConfig): Promise<OperationResult> =>
    ipcRenderer.invoke('workspace-test-connection', gitConfig),
  ...
  workspaceOpenFolder: (workspaceId: string): Promise<OperationResult> =>
  ...
  onEnvironmentsStructureChanged: (
    callback: (data: { workspaceId: string; timestamp: number }) => void,
  ): (() => void) => {
  ...
  onShowErrorMessage: (callback: (errorData: { title?: string; message: string }) => void): (() => void) => {
    const subscription = (_event: IpcRendererEvent, errorData: { title?: string; message: string }) =>
      callback(errorData);
  ...
  generateTeamWorkspaceInvite: (
    workspaceData: Partial<Workspace> & { includeAuthData?: boolean },
  ): Promise<GenerateInviteResult> => ipcRenderer.invoke('generate-team-workspace-invite', workspaceData),

  // Generate environment config links
  generateEnvironmentConfigLink: (environmentData: EnvironmentLinkData): Promise<GenerateEnvLinkResult> =>
    ipcRenderer.invoke('generate-environment-config-link', environmentData),
