...
import { BasicAuthStrategy } from '@/services/workspace/git/auth/BasicAuthStrategy';
...
describe('BasicAuthStrategy', () => {
  ...
  describe('embedCredentials()', () => {
    it('embeds username and password into HTTPS URL', () => {
      ...
      expect(result).toBe('https://deploy-bot:secure-password-123@gitlab.openheaders.io/platform/shared-headers.git');
    });
    ...
    it('handles Unicode characters in credentials', () => {
      const result = strategy.embedCredentials('https://gitlab.openheaders.io/repo.git', 'müller', 'pässwörd');
      const parsed = new URL(result);
    ...
    it('throws for invalid URL', () => {
      expect(() => strategy.embedCredentials('not-a-url', 'user', 'pass')).toThrow('Failed to parse Git URL');
    });
  });
  ...
  describe('setup()', () => {
    it('returns effectiveUrl with embedded credentials and process.env', async () => {
      const result = await strategy.setup('https://gitlab.openheaders.io/platform/shared-headers.git', {
        username: 'deploy-bot',
        password: 'ghp_aBcDeFgHiJkLmNoPqRsT',
      });
    ...
    it('throws when username is missing', async () => {
      await expect(strategy.setup('https://gitlab.openheaders.io/repo.git', { password: 'pass' })).rejects.toThrow(
        'Username and password are required',
      );
    });

    it('throws when password is missing', async () => {
      await expect(strategy.setup('https://gitlab.openheaders.io/repo.git', { username: 'user' })).rejects.toThrow(
        'Username and password are required',
      );
    });

    it('throws when both missing', async () => {
      await expect(strategy.setup('https://gitlab.openheaders.io/repo.git', {})).rejects.toThrow(
        'Username and password are required',
      );
    });
  });
