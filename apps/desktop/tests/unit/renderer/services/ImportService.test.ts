...
describe('ImportService._validateImportOptions', () => {
  it('throws for null options', () => {
    ...
    expect(() => service._validateImportOptions(null as unknown as ImportOptions)).toThrow(
      'Import options must be provided',
    );
  });
  ...
  it('throws for non-object options', () => {
    const service = new ImportService(makeDeps());
    // intentionally passing invalid input to test runtime validation
    expect(() => service._validateImportOptions('str' as unknown as ImportOptions)).toThrow(
      'Import options must be provided',
    );
  });
...
describe('ImportService._handleWorkspaceImport', () => {
  ...
  it('uses workspaceInfo from options when present', async () => {
    const service = new ImportService(makeDeps());
    const importSpy = vi
      .spyOn(service.workspaceHandler, 'importWorkspace')
      .mockResolvedValue({ createdWorkspace: { name: 'WS', type: 'git' }, errors: [] });

    await service._handleWorkspaceImport({}, { workspaceInfo: { name: 'WS', type: 'git' } } as ImportOptions);

    expect(importSpy).toHaveBeenCalledWith({ name: 'WS', type: 'git' }, expect.any(Object));
  });
  ...
  it('falls back to importData.workspace', async () => {
    ...
    await service._handleWorkspaceImport({ workspace: { name: 'WS2', type: 'git' } }, {} as ImportOptions);

    expect(importSpy).toHaveBeenCalledWith({ name: 'WS2', type: 'git' }, expect.any(Object));
  });
});
