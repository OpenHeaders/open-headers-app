    ...
    if (node.tagName === 'link' && node.attributes?._cssText) {
      const cssBaseUrl = node.attributes.href
        ? node.attributes.href.startsWith('http')
          ...
          : resolveRelativeUrl(node.attributes.href, baseUrl) || baseUrl
        : baseUrl;
 ...
 */
async function preprocessRecordingForSave(
  recordingData: PreprocessorData,
  options: PreprocessOptions = {},
): Promise<PreprocessorData> {
  ...
  try {
    ...
    for (let i = 0; i < record.events.length; i++) {
      ...
      if (i > 0 && i % 20 === 0 && options.onProgress) {
        ...
        options.onProgress('preprocessing', progress, {
          ...
          totalEvents: record.events.length,
          resourcesNormalized:
            staticResources.scripts.size + staticResources.stylesheets.size + staticResources.images.size,
        });
      }
      ...
      if (event.type === 'rrweb' && event.data) {
        ...
        if (rrwebEvent.type === 2) {
          ...
          const processedData = preprocessSnapshot(
            rrwebEvent.data as Snapshot,
            fontUrls,
            staticResources,
            effectiveBaseUrl,
            resourceMap,
    ...
    log.info(
      `Preprocessing complete: ${processedEvents.length} events, ${fontUrls.size} fonts, ${staticResources.scripts.size + staticResources.stylesheets.size + staticResources.images.size + staticResources.fonts.size + staticResources.other.size} total resources`,
    );
    log.info(
      `Preprocessing timing: First pass: ${firstPassDuration}ms, Second pass: ${secondPassDuration}ms, Total: ${totalPreprocessingDuration}ms`,
    );
    ...
    if (options.onProgress) {
      options.onProgress('preprocessing', 100, {
        ...
        totalResources:
          staticResources.scripts.size +
          staticResources.stylesheets.size +
          staticResources.images.size +
          staticResources.fonts.size +
          staticResources.other.size,
      });
    }
...
function processIncrementalSnapshot(
  event: PreprocessorEvent | RRWebEvent,
  fontUrls: Set<string>,
  ...
  if (data.source === 0 && data.adds) {
    ...
    if (isMouseEvent) {
      ...
      const nextPageIndex = pageTransitions.findIndex((p) => p.timestamp > eventTimestamp);
 ...
 */
function extractFontUrlsFromCss(
  cssText: string,
  fontUrls: Set<string>,
...
async function prefetchResources(
  ...
  staticResources.stylesheets.forEach((url) => {
    allResources.push({ url, type: 'stylesheet', priority: 1 });
  });
  staticResources.fonts.forEach((url) => {
    allResources.push({ url, type: 'font', priority: 2 });
  });
  staticResources.scripts.forEach((url) => {
    allResources.push({ url, type: 'script', priority: 3 });
  });
  staticResources.images.forEach((url) => {
    allResources.push({ url, type: 'image', priority: 4 });
  });
  staticResources.other.forEach((url) => {
    allResources.push({ url, type: 'other', priority: 5 });
  });
  ...
  log.info(
    `Prefetch complete: ${completed} succeeded, ${failed} failed out of ${allResources.length} total (took ${prefetchDuration}ms)`,
  );
}
...
export type { PreprocessorData, PreprocessorEvent, PreprocessorRecord };
