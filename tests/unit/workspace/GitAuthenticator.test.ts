...
describe('GitAuthenticator', () => {
    ...
    describe('setupAuth()', () => {
        ...
        describe('unknown type', () => {
            it('throws for unknown auth type "kerberos"', async () => {
                await expect(auth.setupAuth(GITHUB_URL, 'kerberos'))
                    .rejects.toThrow('Unknown authentication type: kerberos');
            });

            it('throws for unknown auth type "oauth"', async () => {
                await expect(auth.setupAuth(GITHUB_URL, 'oauth'))
                    .rejects.toThrow('Unknown authentication type: oauth');
            });
        });
        ...
        describe('error wrapping', () => {
            it('wraps strategy errors with auth type prefix', async () => {
                await expect(
                    auth.setupAuth(GITHUB_URL, 'token', {})
                ).rejects.toThrow('token authentication setup failed');
            });

            it('wraps basic auth missing creds error', async () => {
                await expect(
                    auth.setupAuth(GITHUB_URL, 'basic', {})
                ).rejects.toThrow('basic authentication setup failed');
            });
        });
    });
    ...
    describe('getSafeDisplayUrl()', () => {
        it('appends "(with token authentication)" for token type', () => {
            expect(auth.getSafeDisplayUrl(GITHUB_URL, 'token'))
                .toBe(`${GITHUB_URL} (with token authentication)`);
        });

        it('appends "(with username/password)" for basic type', () => {
            expect(auth.getSafeDisplayUrl(GITHUB_URL, 'basic'))
                .toBe(`${GITHUB_URL} (with username/password)`);
        });

        it('appends "(with SSH key)" for ssh-key type', () => {
            expect(auth.getSafeDisplayUrl(GITHUB_URL, 'ssh-key'))
                .toBe(`${GITHUB_URL} (with SSH key)`);
        });
